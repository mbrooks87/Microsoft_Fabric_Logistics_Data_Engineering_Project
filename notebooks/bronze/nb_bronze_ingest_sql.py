...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_order_lines.csv',
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_customers.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_products.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_inventory.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_inventory_transactions.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_shipments.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_carriers.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_warehouses.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_locations.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_purchase_orders.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_suppliers.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_supplier_performance.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_returns.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_employees.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_labor_shifts.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_dock_activity.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
# NOTE: enableChangeDataFeed = true — required for Part 4 streaming reads
# ─────────────────────────────────────────────────────────────────────────────
spark.sql("""
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_iot_events.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
...
AS
SELECT *, current_timestamp() AS _loaded_at
FROM read_files('Files/raw/raw_date_dim.csv',
    format => 'csv', header => 'true', inferSchema => 'true', nullValue => '')
WHERE date_key IS NOT NULL
""")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 21 — Bronze Validation: UNION ALL audit across all 19 tables
# All status values must read 'OK' before proceeding to Part 2
# ─────────────────────────────────────────────────────────────────────────────
spark.sql("""
WITH audit AS (
    SELECT 'raw_orders' AS tbl, 'order_id' AS pk, COUNT(*) AS rows,
           SUM(CASE WHEN order_id IS NULL THEN 1 ELSE 0 END) AS pk_nulls
    FROM bronze.raw_orders
    UNION ALL SELECT 'raw_order_lines','line_id',COUNT(*),
        SUM(CASE WHEN line_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_order_lines
    UNION ALL SELECT 'raw_customers','customer_id',COUNT(*),
        SUM(CASE WHEN customer_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_customers
    UNION ALL SELECT 'raw_products','sku',COUNT(*),
        SUM(CASE WHEN sku IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_products
    UNION ALL SELECT 'raw_inventory','inventory_id',COUNT(*),
        SUM(CASE WHEN inventory_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_inventory
    UNION ALL SELECT 'raw_inventory_transactions','txn_id',COUNT(*),
        SUM(CASE WHEN txn_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_inventory_transactions
    UNION ALL SELECT 'raw_shipments','shipment_id',COUNT(*),
        SUM(CASE WHEN shipment_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_shipments
    UNION ALL SELECT 'raw_carriers','carrier_id',COUNT(*),
        SUM(CASE WHEN carrier_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_carriers
    UNION ALL SELECT 'raw_warehouses','warehouse_id',COUNT(*),
        SUM(CASE WHEN warehouse_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_warehouses
    UNION ALL SELECT 'raw_locations','location_id',COUNT(*),
        SUM(CASE WHEN location_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_locations
    UNION ALL SELECT 'raw_purchase_orders','po_id',COUNT(*),
        SUM(CASE WHEN po_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_purchase_orders
    UNION ALL SELECT 'raw_suppliers','supplier_id',COUNT(*),
        SUM(CASE WHEN supplier_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_suppliers
    UNION ALL SELECT 'raw_supplier_performance','perf_id',COUNT(*),
        SUM(CASE WHEN perf_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_supplier_performance
    UNION ALL SELECT 'raw_returns','return_id',COUNT(*),
        SUM(CASE WHEN return_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_returns
    UNION ALL SELECT 'raw_employees','employee_id',COUNT(*),
        SUM(CASE WHEN employee_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_employees
    UNION ALL SELECT 'raw_labor_shifts','shift_id',COUNT(*),
        SUM(CASE WHEN shift_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_labor_shifts
    UNION ALL SELECT 'raw_dock_activity','dock_id',COUNT(*),
        SUM(CASE WHEN dock_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_dock_activity
    UNION ALL SELECT 'raw_iot_events','event_id',COUNT(*),
        SUM(CASE WHEN event_id IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_iot_events
    UNION ALL SELECT 'raw_date_dim','date_key',COUNT(*),
        SUM(CASE WHEN date_key IS NULL THEN 1 ELSE 0 END)
    FROM bronze.raw_date_dim
)
SELECT tbl, pk AS primary_key, rows, pk_nulls,
