-- ============================================================
-- nb_silver_transform.sql
-- WMS SQL-First Medallion Architecture
-- Part 2 of 4: Silver Transform
-- 15 tables · ROW_NUMBER() dedup · CTE chains · 0 PySpark
-- ============================================================
...
SELECT 'silver.silver_orders' AS tbl, COUNT(*) AS rows FROM silver.silver_orders;
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY line_id ORDER BY _loaded_at DESC) AS rn
...
SELECT 'silver.silver_order_lines' AS tbl, COUNT(*) AS rows FROM silver.silver_order_lines;
...
SELECT 'silver.silver_customers' AS tbl, COUNT(*) AS rows FROM silver.silver_customers;
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY sku ORDER BY _loaded_at DESC) AS rn
...
SELECT
    ...
    CASE
        WHEN CAST(unit_price AS DOUBLE) > 0
        THEN (CAST(unit_price AS DOUBLE) - CAST(unit_cost AS DOUBLE))
             / CAST(unit_price AS DOUBLE)
        ELSE 0.0
    END                             AS margin_pct,
    -- volume in cubic feet (1 ft³ = 28,316.8 cm³)
    (CAST(length_cm AS DOUBLE)
     * CAST(width_cm AS DOUBLE)
     * CAST(height_cm AS DOUBLE)) / 28316.8 AS volume_cuft,
    current_timestamp()             AS _loaded_at
FROM deduped
WHERE rn = 1;

SELECT 'silver.silver_products' AS tbl, COUNT(*) AS rows FROM silver.silver_products;


-- ------------------------------------------------------------
-- CELL 5: silver_inventory
-- source:  bronze.raw_inventory + bronze.raw_products (lookup)
-- derived: inventory_value, below_reorder, utilization_rate
-- ------------------------------------------------------------
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY inventory_id ORDER BY _loaded_at DESC) AS rn
...
SELECT 'silver.silver_inventory' AS tbl, COUNT(*) AS rows FROM silver.silver_inventory;
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY txn_id ORDER BY _loaded_at DESC) AS rn
...
SELECT
    ...
    -- flag transactions where qty math doesn't reconcile
    CASE
        WHEN CAST(qty_before AS INT) + CAST(qty_change AS INT) <> CAST(qty_after AS INT)
...
SELECT 'silver.silver_inventory_transactions' AS tbl, COUNT(*) AS rows FROM silver.silver_inventory_transactions;
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY shipment_id ORDER BY _loaded_at DESC) AS rn
...
typed AS (
    SELECT
        ...
        CASE
            ...
            ELSE 0
        END                             AS delay_days,
        -- cost efficiency per pound shipped
        CASE
            WHEN CAST(weight_lbs AS DOUBLE) > 0
...
with_carrier AS (
    SELECT
        ...
        c.service_level,
        c.avg_transit_days,
        c.cost_per_lb AS carrier_rate_per_lb
    FROM typed t
    LEFT JOIN bronze.raw_carriers c ON t.carrier_id = c.carrier_id
)
SELECT *, current_timestamp() AS _loaded_at FROM with_carrier;

SELECT 'silver.silver_shipments' AS tbl, COUNT(*) AS rows FROM silver.silver_shipments;
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY po_id ORDER BY _loaded_at DESC) AS rn
...
SELECT 'silver.silver_purchase_orders' AS tbl, COUNT(*) AS rows FROM silver.silver_purchase_orders;
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY return_id ORDER BY _loaded_at DESC) AS rn
...
SELECT
    ...
    disposition,
    CASE
        WHEN restock_date IS NOT NULL
        THEN DATEDIFF(TO_DATE(restock_date), TO_DATE(return_date))
        ELSE NULL
    END                                 AS days_to_restock,
    UPPER(TRIM(condition)) IN ('NEW', 'LIKE_NEW') AS is_resellable,
    CASE
        WHEN UPPER(reason_code) IN ('DAMAGED', 'DEFECTIVE')         THEN 'Quality'
...
SELECT 'silver.silver_returns' AS tbl, COUNT(*) AS rows FROM silver.silver_returns;
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY perf_id ORDER BY _loaded_at DESC) AS rn
...
tiered AS (
    SELECT *,
        CASE
            WHEN composite_score >= 0.95 THEN 'A'
...
SELECT 'silver.silver_supplier_performance' AS tbl, COUNT(*) AS rows FROM silver.silver_supplier_performance;
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY employee_id ORDER BY _loaded_at DESC) AS rn
...
typed AS (
    SELECT
        ...
        -- estimated annual cost: 52 wks × 40 hrs = 2,080 hrs/yr
...
SELECT 'silver.silver_employees' AS tbl, COUNT(*) AS rows FROM silver.silver_employees;
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY shift_id ORDER BY _loaded_at DESC) AS rn
...
typed AS (
    SELECT
        ...
        CASE
            WHEN CAST(hours_worked AS DOUBLE) > 0
...
SELECT 'silver.silver_labor_shifts' AS tbl, COUNT(*) AS rows FROM silver.silver_labor_shifts;
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY dock_id ORDER BY _loaded_at DESC) AS rn
...
SELECT
    ...
    CASE
        WHEN CAST(duration_minutes AS INT) > 0
        THEN CAST(num_pallets AS DOUBLE) / (CAST(duration_minutes AS DOUBLE) / 60.0)
        ELSE NULL
    END                             AS pallets_per_hour,
    UPPER(TRIM(activity_type)) = 'INBOUND_RECEIVE' AS is_inbound,
    current_timestamp()             AS _loaded_at
FROM deduped
WHERE rn = 1;

SELECT 'silver.silver_dock_activity' AS tbl, COUNT(*) AS rows FROM silver.silver_dock_activity;
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY location_id ORDER BY _loaded_at DESC) AS rn
...
SELECT 'silver.silver_locations' AS tbl, COUNT(*) AS rows FROM silver.silver_locations;


-- ------------------------------------------------------------
-- CELL 15: silver_date_dim
-- source:  bronze.raw_date_dim
-- derived: month_label (consistent yyyy-MM for all fact joins)
-- ------------------------------------------------------------
...
WITH deduped AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY date_key ORDER BY _loaded_at DESC) AS rn
...
SELECT
    ...
    -- canonical yyyy-MM label for all Power BI time-series joins
    CONCAT(
...
SELECT 'silver.silver_date_dim' AS tbl, COUNT(*) AS rows FROM silver.silver_date_dim;


-- ------------------------------------------------------------
-- CELL 16: Full Silver Validation
-- pk_nulls + dupe check across all 15 tables
-- All rows must show status = 'OK' before running Gold
-- ------------------------------------------------------------
WITH audit AS (
    SELECT 'silver_orders'                  AS tbl, 'order_id'     AS pk, COUNT(*) AS rows, SUM(CASE WHEN order_id     IS NULL THEN 1 ELSE 0 END) AS pk_nulls, COUNT(*) - COUNT(DISTINCT order_id)     AS dupes FROM silver.silver_orders
    UNION ALL
    SELECT 'silver_order_lines',                     'line_id',             COUNT(*),        SUM(CASE WHEN line_id     IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT line_id)     FROM silver.silver_order_lines
    UNION ALL
    SELECT 'silver_customers',                       'customer_id',         COUNT(*),        SUM(CASE WHEN customer_id IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT customer_id) FROM silver.silver_customers
    UNION ALL
    SELECT 'silver_products',                        'sku',                 COUNT(*),        SUM(CASE WHEN sku         IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT sku)         FROM silver.silver_products
    UNION ALL
    SELECT 'silver_inventory',                       'inventory_id',        COUNT(*),        SUM(CASE WHEN inventory_id IS NULL THEN 1 ELSE 0 END),            COUNT(*) - COUNT(DISTINCT inventory_id) FROM silver.silver_inventory
    UNION ALL
    SELECT 'silver_inventory_transactions',          'txn_id',              COUNT(*),        SUM(CASE WHEN txn_id      IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT txn_id)      FROM silver.silver_inventory_transactions
    UNION ALL
    SELECT 'silver_shipments',                       'shipment_id',         COUNT(*),        SUM(CASE WHEN shipment_id IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT shipment_id) FROM silver.silver_shipments
    UNION ALL
    SELECT 'silver_purchase_orders',                 'po_id',               COUNT(*),        SUM(CASE WHEN po_id       IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT po_id)       FROM silver.silver_purchase_orders
    UNION ALL
    SELECT 'silver_returns',                         'return_id',           COUNT(*),        SUM(CASE WHEN return_id   IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT return_id)   FROM silver.silver_returns
    UNION ALL
    SELECT 'silver_supplier_performance',            'perf_id',             COUNT(*),        SUM(CASE WHEN perf_id     IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT perf_id)     FROM silver.silver_supplier_performance
    UNION ALL
    SELECT 'silver_employees',                       'employee_id',         COUNT(*),        SUM(CASE WHEN employee_id IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT employee_id) FROM silver.silver_employees
    UNION ALL
    SELECT 'silver_labor_shifts',                    'shift_id',            COUNT(*),        SUM(CASE WHEN shift_id    IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT shift_id)    FROM silver.silver_labor_shifts
    UNION ALL
    SELECT 'silver_dock_activity',                   'dock_id',             COUNT(*),        SUM(CASE WHEN dock_id     IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT dock_id)     FROM silver.silver_dock_activity
    UNION ALL
    SELECT 'silver_locations',                       'location_id',         COUNT(*),        SUM(CASE WHEN location_id IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT location_id) FROM silver.silver_locations
    UNION ALL
    SELECT 'silver_date_dim',                        'date_key',            COUNT(*),        SUM(CASE WHEN date_key    IS NULL THEN 1 ELSE 0 END),             COUNT(*) - COUNT(DISTINCT date_key)    FROM silver.silver_date_dim
)
SELECT
    tbl,
    pk      AS primary_key,
